...
if (isset($_GET['id'])) {
    ...
    // Query untuk mendapatkan data rekom berdasarkan id
    $sql = "SELECT * FROM rekom WHERE id = $id";
    ...
    if ($result->num_rows == 1) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $sql_update = "UPDATE rekom SET title='$new_title', sub_title='$new_sub_title', description='$new_description' WHERE id=$id";

            if ($conn->query($sql_update) === TRUE) {
                // Jika perubahan berhasil disimpan, redirect ke halaman rekom
...
<div class="content-wrapper">
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Edit Judul Section rekom</h4>
                    <p class="card-description">Edit Judul, Sub Judul, Deskripsi</p>
                    <form action="?q=edit_rekom&id=<?php echo $id; ?>" method="POST">
                        ...
                        <a href="?q=rekom" class="btn btn-secondary">Batal</a>
                    </form>
                </div>
            </div>
        </div>
